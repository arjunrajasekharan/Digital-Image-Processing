...
def get_resized_arr(old_shape, factor):
  ...
  new_arr = numpy.zeros([dim1, dim2, 3], dtype=numpy.uint8)
  return new_arr
...
def main(): 
  ...
  cv2.imshow('lena_halffactored', new_arr1)
  cv2.imshow('lena_oncefactored', new_arr2)
  cv2.imshow('lena_twicefactored', new_arr3)
  cv2.imshow('lena_builtin_halffactored', builtin_half)
  cv2.imshow('lena_builtin_oncefactored', builtin_once)
  cv2.imshow('lena_builtin_twicefactored', builtin_twice)
  print("Enter any key to close all windows: ")
  cv2.waitKey(0)
